...
- Rector rewrites on `make rector`: adds `#[\Override]` to overridden methods, adds `#[\SensitiveParameter]` to secret-named params (`password`, `apiToken`, ...) — generate code that way up front. Full builder behavior in `docs/php/Rector.md`.
