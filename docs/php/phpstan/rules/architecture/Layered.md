...
A classic three-layer separation: the `Domain` layer may not depend on either `Application` or `Infrastructure`, and the `Application` layer may not depend on `Infrastructure`. Serves both as the foundation that `ddd()` builds on and as a standalone preset for projects that want layered isolation without the rest of the DDD machinery.
...
The same shape catches `Application → Infrastructure` leaks: an `Acme\Application\*` class importing from `Acme\Infrastructure\*` triggers the rule.
