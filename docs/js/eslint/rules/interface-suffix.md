...
Parents that do not end with `Interface` — a base class, a marker type, an unsuffixed interface
— are ignored for the count. They neither trigger nor suppress the check.

## Differences from the PHP rule

- TypeScript is structurally typed, so an `implements` clause is optional.
  The rule sees only explicit clauses and cannot judge a class that satisfies a contract without naming it.
- an implemented name may be qualified (`Acme.UserRepositoryInterface`); the comparison uses its last segment
- TypeScript allows `implements` on a type alias as well as on an interface.
  Both are treated alike, since the name is what carries the convention.
